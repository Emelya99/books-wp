	...
	var JetCW = {
		...
		init: function () {
			...
			$(document)
				...
				.on( 'jet-engine/listing-grid/after-lazy-load', self.setCWSettingsAfterAjaxLoad )
				.on( 'jet-woo-builder-content-rendered', self.setCWSettingsAfterAjaxLoad );

			$( window ).on( 'jet-popup/render-content/ajax/success', self.prepareJetPopup );

		},

		setCWSettingsAfterAjaxLoad: function ( event, scope, response ) {
			let cwData = response.data.compare_wishlist_data;

			if ( JetCWSettings.widgets.wishlist && cwData.wishlist ) {
				Object.assign( JetCWSettings.widgets.wishlist, cwData.wishlist );
			}

			if ( JetCWSettings.widgets.compare && cwData.compare ) {
				Object.assign( JetCWSettings.widgets.compare, cwData.compare );
			}
		},
