...
if ( ! class_exists( 'Jet_CW_Integration' ) ) {
	...
	class Jet_CW_Integration {
		...
		public function __construct() {
			...
			add_filter( 'jet-engine/ajax/get_listing/response', [ $this, 'add_to_jet_engine_response_cw_data' ], 10, 2 );
			add_filter( 'jet-woo-builder/ajax-handler/get-switcher-template/response', [ $this, 'add_to_jet_woo_builder_response_cw_data' ] );

		}

		/**
		 * Add the cw data to JetEngine response data.
		 *
		 ...
		 * @return array
		 */
		public function add_to_jet_engine_response_cw_data( $response, $widget_settings ) {
			if ( empty( $widget_settings['lazy_load'] ) ) {
				return $response;
			}

			$cw_data = jet_cw()->widgets_store->widgets_types;

			if ( ! empty( $cw_data ) ) {
				$response['compare_wishlist_data'] = $cw_data;
			}
		...
		/**
		 * Add the cw data to JetWooBuilder response data.
		 *
		 * @param array $response
		 *
		 * @return array
		 */
		public function add_to_jet_woo_builder_response_cw_data( $response ) {
			$cw_data = jet_cw()->widgets_store->widgets_types;
